...
class RepositoryDetailsViewModelTest : TestCase() {
    ...
    */
    ...
    /*
    * Проверяем обработку события возврата подключения к интернету
    * Ожидаем что будет установлен параметр hasInternetConnection = true во viewState
    * В effects должен появится новый объект на перезагрузку страницы
    */
    ...
    * Ожитаем эффект на перезагрузку страницы не должен был добавиться в effects
    */
    ...
    /*
    * Проверяем последовательность событий на получение детальной информации о репозитории
    * Ожидаем что в viewState добавлена информация о репозитории которую мы передали
    * Была иннициирована загрузка данных о владельце по средствам вызова метода getUserInfo
    */
    ...
    /*
    * Проверяем последовательность событий на получение детальной информации о репозитории,
    * если не был передан id владельца репозитория
    * Ожидаем что в viewState добавлена информация о репозитории, которую мы передали
    * НЕ была иннициирована загрузка данных о владельце по средствам вызова метода getUserInfo
    ...
    */
    ...
    /*
    * Проверяем последовательность событий на на получение детальной информации о репозитории,
    ...
    */
    ...
    /*
    * Проверяем последовательность событий на получение детальной информации о владельце репозитория
    ...
    */
    ...
    /*
    * Проверяем последовательность событий на getInformationErrorEvent
